...
*** Chinolor Theme
This matches waybar theme better - but has some glitches I can't be bothered resolving (e.g. TODO items in org-mode have boxes around them).

#+begin_src emacs-lisp
;;(load "~/.emacs.d/chinolor-theme.el" nil t)
;;(load-theme 'chinolor t)
...
Testing without line numbers for now.. but a little margin..
...
#+RESULTS:
: ((swiper . ivy--regex-plus) (swiper-isearch . regexp-quote) (counsel-ag . ivy--regex-plus) (counsel-rg . ivy--regex-plus) (t . ivy--regex-fuzzy))
...
#+RESULTS:
| d | Daily agenda and all TODOs | ((tags PRIORITY="A" ((org-agenda-skip-function '(org-agenda-skip-entry-if 'todo 'done)) (org-agenda-overriding-header High-priority unfinished tasks:))) (agenda ) (alltodo  ((org-agenda-skip-function '(or (air-org-skip-subtree-if-priority 65) (org-agenda-skip-if nil '(scheduled deadline))))))) |
...
** Enabling Org Bullets and fonts with org-superstar
...
(set-face-attribute 'org-level-8 nil :weight 'normal :inherit 'variable-pitch :foreground "#5E81AC")
(set-face-attribute 'org-level-7 nil :inherit 'org-level-8 :weight 'normal :height 1.02 :foreground "#6B8BB3")
(set-face-attribute 'org-level-6 nil :inherit 'org-level-8 :weight 'normal :height 1.03 :foreground "#7896BA")
(set-face-attribute 'org-level-5 nil :inherit 'org-level-8 :weight 'normal :height 1.04 :foreground "#86A1C1")
(set-face-attribute 'org-level-4 nil :inherit 'org-level-8 :weight 'normal :height 1.05 :foreground "#93ABC8")
(set-face-attribute 'org-level-3 nil :inherit 'org-level-8 :weight 'normal :height 1.1 :foreground "#A1B5CE")
(set-face-attribute 'org-level-2 nil :inherit 'org-level-8 :weight 'normal :height 1.15 :foreground "#AEC0D5")
(set-face-attribute 'org-level-1 nil :inherit 'org-level-8 :weight 'normal :height 1.2 :foreground "#BCCADC")
...
#+RESULTS:
: ((TODO . 63024) (PROJ . 61717) (DOING . 61641) (STALLED . 63198) (TESTING . 59217) (CANCELLED . 63063) (DONE . 63028))
...
**** Show back-links in open

This code doesn't work - neither arch nor mac os.
See: https://www.gitmemory.com/issue/org-roam/org-roam/1669/886137520
Even though code below comes after loading org-roam, org-roam seems to be lazy loaded and org-roam-node doesn't exist until org-roam is first triggered..
...
** Org ox-rst (Restructured Text exporting)
...
(setq mu4e-contexts
  (list
   (make-mu4e-context
    :name "Dixon" ;;for dixon-gmail
    :enter-func (lambda () (mu4e-message "Entering context work"))
    :leave-func (lambda () (mu4e-message "Leaving context work"))
    :match-func (lambda (msg)
		  (when msg
		(mu4e-message-contact-field-matches
		 msg '(:from :to :cc :bcc) "mark@dixon.se")))
    :vars '((user-mail-address . "mark@dixon.se")
	    (user-full-name . "Mark Dixon")
	    (mu4e-sent-folder . "/dixon-gmail/[dixon].Sent Mail")
	    (mu4e-drafts-folder . "/dixon-gmail/[dixon].drafts")
	    (mu4e-trash-folder . "/dixon-gmail/[dixon].Trash")
	    (mu4e-compose-signature . (concat "Formal Signature\n" "Emacs 25, org-mode 9, mu4e 1.0\n"))
	    (mu4e-compose-format-flowed . t)
	    (smtpmail-queue-dir . "~/Maildir/dixon-gmail/queue/cur")
	    (message-send-mail-function . smtpmail-send-it)
	    (smtpmail-smtp-user . "mark@dixon.se")
	    ...
	    (mu4e-maildir-shortcuts . ( ("/dixon-gmail/INBOX"            . ?i)
					("/dixon-gmail/[dixon].Sent Mail" . ?s)
					("/dixon-gmail/[dixon].Trash"       . ?t)
					("/dixon-gmail/[dixon].All Mail"  . ?a)
					("/dixon-gmail/[dixon].Starred"   . ?r)
					("/dixon-gmail/[dixon].drafts"    . ?d)
					))))
   (make-mu4e-context
    :name "Nitwit" ;;for acc2-gmail
    :enter-func (lambda () (mu4e-message "Entering context personal"))
    :leave-func (lambda () (mu4e-message "Leaving context personal"))
    :match-func (lambda (msg)
		  (when msg
		(mu4e-message-contact-field-matches
		 msg '(:from :to :cc :bcc) "mark@nitwit.se")))
    :vars '((user-mail-address . "mark@nitwit.se")
	    (user-full-name . "Mark Dixon")
	    (mu4e-sent-folder . "/nitwit-gmail/[nitwit].Sent Mail")
	    (mu4e-drafts-folder . "/nitwit-gmail/[nitwit].drafts")
	    (mu4e-trash-folder . "/nitwit-gmail/[nitwit].Trash")
	    (mu4e-compose-signature . (concat "Informal Signature\n" "Emacs is awesome!\n"))
	    (mu4e-compose-format-flowed . t)
	    (smtpmail-queue-dir . "~/Maildir/nitwit-gmail/queue/cur")
	    (message-send-mail-function . smtpmail-send-it)
	    (smtpmail-smtp-user . "mark@nitwit.se")
	    (smtpmail-starttls-credentials . (("smtp.gmail.com" 587 nil nil)))
	    (smtpmail-auth-credentials . (expand-file-name  "~/.authinfo.gpg"))
	    (smtpmail-default-smtp-server . "smtp.gmail.com")
	    (smtpmail-smtp-server . "smtp.gmail.com")
	    ...
	    (smtpmail-debug-info . t)
	    (smtpmail-debug-verbose . t)
	    (mu4e-maildir-shortcuts . ( ("/nitwit-gmail/INBOX"            . ?i)
					("/nitwit-gmail/[nitwit].Sent Mail" . ?s)
					("/nitwit-gmail/[nitwit].Trash"     . ?t)
					("/nitwit-gmail/[nitwit].All Mail"  . ?a)
					("/nitwit-gmail/[nitwit].Starred"   . ?r)
					("/nitwit-gmail/[nitwit].drafts"    . ?d)
					))))))
#+end_src

#+RESULTS:
| #s(mu4e-context Dixon (lambda nil (mu4e-message Entering context work)) (lambda nil (mu4e-message Leaving context work)) (lambda (msg) (when msg (mu4e-message-contact-field-matches msg '(:from :to :cc :bcc) mark@dixon.se))) ((user-mail-address . mark@dixon.se) (user-full-name . Mark Dixon) (mu4e-sent-folder . /dixon-gmail/[dixon].Sent Mail) (mu4e-drafts-folder . /dixon-gmail/[dixon].drafts) (mu4e-trash-folder . /dixon-gmail/[dixon].Trash) (mu4e-compose-signature concat Formal Signature |



** Make things look and function a bit nicer

Allow for different colors for each column in mu4e-headers view:
